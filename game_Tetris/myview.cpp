...
//     {
//         QRectF rect(199, y, 202, 22); // (x, y, width, height)
//         QList<QGraphicsItem*> list = scene()->items(rect, Qt::ContainsItemShape);
//         // QList<QGraphicsItem*> list = scene()->items(199, y, 202, 22, Qt::ContainsItemShape); 函数中的rect为const
//         // 如果该行已满
//         if (list.count() == 10)
//         {
...
void MyView::moveBox()
{
    ...
    {
        int row = rows.at(i - 1);
        QRectF rect(199, 49, 202, row - 47); // (x, y, width, height)
        foreach (QGraphicsItem* item, scene()->items(rect, Qt::ContainsItemShape)) //错误如上
        {
            item->moveBy(0, 20);
        }
    }
